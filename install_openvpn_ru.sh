...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	clear
	echo 'Добро пожаловать в установку OpenVPN сервера!'
	...
	if [[ $(ip -4 addr | grep inet | grep -vEc '127(\.[0-9]{1,3}){3}') -eq 1 ]]; then
		...
		echo
		echo "Какой адрес IPv4 следует использовать?"
	...
	if echo "$ip" | grep -qE '^(10\.|172\.1[6789]\.|172\.2[0-9]\.|172\.3[01]\.|192\.168)'; then
		echo
		echo "Этот сервер находится за NAT. Нужно указать внешний ip к которому клиенты будут подключаться?"
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo
	echo "Конфигурация доступна в :" ~/"$client.ovpn"
	...
	case "$option" in
		1)
			echo
			echo "Укажите имя для клиента [0-9 a-z A-Z _-]:"
			read -p "Имя: " unsanitized_client
			client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$unsanitized_client")
			while [[ -z "$client" || -e /etc/openvpn/server/easy-rsa/pki/issued/"$client".crt ]]; do
				echo "$client: Неправильное имя."
				read -p "Имя: " unsanitized_client
				client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$unsanitized_client")
			done
			cd /etc/openvpn/server/easy-rsa/
			...
			new_client
			echo
			echo "$client добавлен. Конфигурация доступна в :" /etc/openvpn/client/"$client.ovpn"
			exit
		;;
		2)
